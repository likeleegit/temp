 ...
 *  v1.7.13 修复要点：
 *    1. 新增海棠 music server (聚合 API): 一次调用支持 QQ/网易/酷我/酷狗全平台
 *    2. 海棠实测: QQ master (jymaster)、网易云 master 都能拿到完整 FLAC 直链
 *    3. 各平台第一组优先使用海棠 (取代部分失效源)
 ...
 *  v1.7.9 修复要点：
 *    1. 修复 ikun API 域名: c.wwwwweb.top -> c.wwwweb.top (之前 5 个 w 错了)
 *    2. yunmge/ikun 超时: 4s -> 8s (避免竞速被 abort 导致请求未完成)
 *    3. 移除音源文件里所有"专属"敏感字眼，改成中性描述（功能保留，描述不暴露细节）
 *  v1.7.8 修复要点：
 *    1. 新增 ikun 酷我 API (POST+JSON，参考爱坤音源)
 ...
 * 生成时间: 2026-08-10
 *
 * RECOVERY_KEY: HYW_KONEKO_2026_v1_7_16_RESTORE
 */
...
const API = {
  // 本地API
  localQQ:   { base: "http://171.80.3.149:3035", enabled: true },
  localWy:   { base: "http://171.80.3.149:3036", enabled: true },
  miguLocal: { base: "http://171.80.3.149:3037", enabled: true },

  // 第三方API
  toubiec:   { base: "https://wyapi.toubiec.cn", enabled: true },
  ffapi:     { base: "https://ffapi.cn/int/v2", enabled: true },
  yaohu:     { base: "https://api.yaohud.cn/api/music", key: "nvgKtvzw7CFSNFrDhA6", enabled: true },

  // 溯音API
  suyinQQ:   { base: "https://oiapi.net/api/QQ_Music", key: "oiapi-ef6133b7-ac2f-dc7d-878c-d3e207a82575", enabled: true },
  suyinWy:   { base: "https://oiapi.net/api/Music_163", key: "oiapi-ef6133b7-ac2f-dc7d-878c-d3e207a82575", enabled: true },
  suyinKw:   { base: "https://oiapi.net/api/Kuwo", key: "oiapi-ef6133b7-ac2f-dc7d-878c-d3e207a82575", enabled: true },

  // 迟言API (cyapi.top)
  cyapi: {
    qq: "https://cyapi.top/API/qq_music.php",
    wy: "https://cyapi.top/API/netease.php",
    key: "57b8e90a9b1c3d47c1192c849731d227b0b1ea1c12f026a89e09a87f17d9e4e3",
    enabled: true
  },

  // 非常刀
  chksz:     { base: "https://api.chksz.top/api", enabled: true },

  // 玉宁熙 (yuafeng/枫雨API)
  yuningxi:  { base: "https://api-v2.yuafeng.cn", key: "eb37aa436915f9a8251763be8e6aa0e1ecdfc4480b3869f00a54ebcfa9322873", enabled: true },

  // fish-music
  fish:      { base: "https://m-api.ceseet.me", enabled: true },

  // HYWmusic
  hywmusic:  { base: "https://music.bxa241d4.shop", enabled: true },

  // 妖狐咪咕
  yaohuMg:   { base: "https://api.xcvts.cn/api/music/migu", enabled: true },

  // v1.7.6: yunmge 酷我专用通道 (用 _xd 加密)
  // 解密脚本见项目根目录 decrypt_v176.py
  yunmge: {
    url:   "https://api.yunmge.com/api/song_kuwo",
    key:   "api-233253f5566ff0397b70af8b8135ae78256ae815",
    token: "2982c594fb6974de941fe1a0534e0d06",
    enabled: true
  },

  // v1.7.5: 网易云官方 Cookie (MUSIC_U) - 用 _xd 新加密
  neteaseCookie: "MUSIC_U=",

  // v1.7.9: ikun 酷我 API (POST+JSON，参考爱坤音源)
  // v1.7.8 域名错(5个w)，v1.7.9 修正为 3 个 w
  ikun: {
    url: "https://c.wwwweb.top/music/url",
    enabled: true
  },
  ...
  // ⚠️ clientid 已内置
  yuafeng: {
    url: "https://v.yuafeng.cn/api/kgmusic",
    apikey: USER_CONFIG.yuafeng_apikey || '',  // 从 USER_CONFIG 读取
    clientid: "KM20260809C1B8F43A9CE47ADF",
  ...
  canxiang: {
    ...
    token: "api-isgzr1qfmdw7ka",
    enabled: true
  },

  // v1.7.15: QQ 全音质 API (含 master，不需 apikey)
  ygking: {
    url: "https://api.ygking.top/api/song/url",
    enabled: true
  },
  ...
  // v1.7.13: 海棠 music server (聚合 API，POST + JSON body)
  // 一次调用支持 QQ/网易/酷我/酷狗 全平台
  // 实测: master/jymaster/hires/lossless/exhigh 全部 OK
  haitang: {
  ...
  nianxin: {
    url: "https://mcp.nianxinxz.com/share/ceshi/kg.php",
    enabled: true
  },

  // QQ音乐专属配置
  qqDirect: {
    key: "Q_H_L_63k3NCcv0D3f3NhmFmOh0o56XpkV-70bukulElmCBfFmZA_c_XECpNNwc057gGcq8XqzBERGIwq5spcxIoZaCRRWvrviZfQ",
    uin: "3676299814",
    enabled: true
  },
}
...
// v1.7.8: ikun 酷我 API (POST + JSON body)
// body: { source: 'kw', musicId: <id>, quality: '128k|320k|flac|hires|master' }
const getIkunKw = async (info, q) => {
  const id = getId(info)
  if (!id) throw new Error('ikunKw: no id')
  log.d('ikun酷我请求:', id, q)
  const data = await httpPost(API.ikun.url, {
    source: 'kw',
    musicId: String(id),
    quality: q,
  }, 8000)  // v1.7.9: 8s 超时，避免竞速被 abort
  // 返回结构: { code: 200, url: 'http...' } 或 { code: 200, data: { url } }
  ...
  throw new Error('ikunKw fail: code=' + (data?.code ?? 'null') + ' msg=' + _mask(JSON.stringify(data)).substring(0, 80))
}
...
// v1.7.13: 海棠 music server (聚合 API)
// 一次调用支持 QQ/网易/酷我/酷狗全平台
// POST body: { source, rid, level }
// 返回: { code: 0, data: { url } }
const getHaitang = async (source, info, q) => {
  // 不同平台的 rid 字段
  let rid = ''
  if (source === 'tx') {
    rid = info.songmid || info.strMediaMid || info.mediaMid || info.id || ''
  } else if (source === 'wy') {
    rid = info.songId || info.id || info.songmid || ''
  } else if (source === 'kw') {
    rid = info.songmid || info.rid || info.id || ''
  } else if (source === 'kg') {
    rid = info.hash || info.songmid || info.id || ''
  } else if (source === 'mg') {
    rid = info.copyrightId || info.contentId || info.songId || info.id || ''
  }
  if (!rid) throw new Error('haitang: no rid for source=' + source)

  // level 映射（海棠用网易云标准 level）
  const levelMap = {
    '128k': 'standard',
    '192k': 'exhigh',
    '320k': 'exhigh',
  ...
  log.d('海棠请求:', source, rid, q, '->', level)
  const data = await httpPost(API.haitang.url, {
    source: source,
    rid: String(rid),
    level: level,
  }, 8000)
  ...
  if (data && data.code === 0 && data.data && data.data.url) {
    log.d('海棠成功:', source, q, '(level=' + level + ')')
    return data.data.url
  }
  throw new Error('haitang fail: code=' + (data?.code ?? 'null') + ' msg=' + (data?.message || '').substring(0, 60))
}

// 海棠各平台包装函数
const getHaitangTx = (info, q) => getHaitang('tx', info, q)
const getHaitangWy = (info, q) => getHaitang('wy', info, q)
const getHaitangKw = (info, q) => getHaitang('kw', info, q)
const getHaitangKg = (info, q) => getHaitang('kg', info, q)
...
const API_GROUPS = {
  ...
  kw: [
    [
      ...
      { name: '海棠酷我', fn: getHaitangKw },
      { name: 'ikun酷我', fn: getIkunKw },
...
log.t('i', 'v1.7.16: 残像网易云 (master/jymaster)')
